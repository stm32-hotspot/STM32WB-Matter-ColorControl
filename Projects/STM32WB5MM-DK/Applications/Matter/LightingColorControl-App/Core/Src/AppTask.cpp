...
void AppTask::LightingActionEventHandler(AppEvent *aEvent) {
    ...
    else if (aEvent->Type == AppEvent::kEventType_Level && aEvent->ButtonEvent.Action != 0) {
    	APP_DBG("aEvent->Type == AppEvent::kEventType_Level");
...
void AppTask::ActionInitiated(LightingManager::Action_t aAction) {
    ...
    if (aAction == LightingManager::ON_ACTION) {
        ...
        if (LightingMgr().IsTurnedOn()) {
            ...
            HsvColor_t mhsv_t  = LightingMgr().GetColorHSV();
            mhsv_t.v = LightingMgr().GetLevel();
            RgbColor_t rbg_t    = HsvToRgb(mhsv_t);
            Led_bar_set_Color(rbg_t.r,rbg_t.g,rbg_t.b);
    ...
    else if (aAction == LightingManager::COLOR_ACTION_HSV) {
    		APP_DBG("COLOR_ACTION_HSV");
            if (LightingMgr().IsTurnedOn()) {
            	 RgbColor_t rbg = LightingMgr().GetColor();
#if (OTA_SUPPORT == 0)

                Led_bar_set_Color(rbg.r,rbg.g,rbg.b);	
#endif
            }
        }
    else if (aAction == LightingManager::COLOR_ACTION_CT) {
    		APP_DBG("COLOR_ACTION_CT");
            if (LightingMgr().IsTurnedOn()) {
            	 RgbColor_t rbg = LightingMgr().GetColor();
#if (OTA_SUPPORT == 0)
