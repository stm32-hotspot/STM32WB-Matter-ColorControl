...
RgbColor_t LightingManager::GetColor() {
    return mrbg;
}

HsvColor_t LightingManager::GetColorHSV() {
    return mhsv;
}
...
bool LightingManager::InitiateAction(Action_t aAction, int32_t aActor, uint16_t size,
    ...
    case COLOR_ACTION_HSV:
    	ChipLogProgress(NotSpecified, "LightMgr:HSV: %u", *value)	
    	;
    	break;
    case COLOR_ACTION_CT:
    	ChipLogProgress(NotSpecified, "LightMgr:CT: %u", *value)	
    	;
    	break;
    ...
    if (aAction == COLOR_ACTION_HSV){
    	mhsv = *reinterpret_cast<HsvColor_t *>(value);
    	mhsv.v = LightingMgr().GetLevel();
    	mrbg = HsvToRgb(mhsv);
    }
    if (aAction == COLOR_ACTION_CT){
    	mct = *reinterpret_cast<CtColor_t *>(value);
    	mrbg = CTToRgb(mct);
    }   
